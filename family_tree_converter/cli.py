...
def main() -> None:
    ...
    diagnostics: dict = {}
    individuals, families = read_spreadsheet(
        args.input, profile_for(args.input), diagnostics)
    ...
    # The report always carries the hardening checks (row coverage, name
    # collisions, the synthetic/placeholder manifest, generation consistency);
    # validation errors/warnings are prepended when present.
    ...
    lines.append("")
    lines.append("=" * 60)
    lines.append("HARDENING CHECKS (review — not necessarily errors)")
    lines.append("=" * 60)
    lines += run_checks(individuals, families, diagnostics)
    ...
    if errors or warnings:
        ...
        print(f"Validation: clean (0 errors, 0 warnings) — see {report_path.name}")
